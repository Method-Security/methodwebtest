// Package cmd implements the CobraCLI commands for the methodwebtest CLI. Subcommands for the CLI should all live within
...
import (
	...
	"github.com/Method-Security/methodwebtest/internal/config"
...
// MethodWebTest is the main struct for the methodwebtest CLI. It contains both the root command and all subcommands that can be
...
// InitRootCommand initializes the root command for the methodwebtest CLI. This command is the parent command for all other
...
func (a *MethodWebTest) InitRootCommand() {
	...
	a.VersionCmd = &cobra.Command{
		Use:   "version",
		Short: "Prints the version number of methodwebtest",
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			return nil
		},
